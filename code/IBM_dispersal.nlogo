...
globals[ ;varibles used by all agents
  count_mort ;Count times of mortality test
...
to import_TREES_SS ;import landscape with all elements (patches, ss and trees)
  ...
  set direct_list sort pathdir:list directory ;list of files in directory in order in ascending order
  set landscape item num_lands direct_list ;get one of the landscape from the list and define as landscape
  set landscape word directory landscape ;set the landscape with the directory to import
end

to import_SS ; import landscape with habitat patches and stepping stones
;-----------------------LANDSCAPE WITH PATCHES AND SS
  set directory word directory_base "exported_ascii_MS_HABMAT_NO_TREES/" ;define directory
  set direct_list sort pathdir:list directory ;list of files in directory in ascending order
  set landscape item num_lands direct_list ;get one of the landscape from the list and define as landscape
  set landscape word directory landscape ;set the landscape with the directory to import
end

to import_TREES ; import landscape with habitat patches and trees
;-----------------------LANDSCAPE WITH PATCHES AND SS
  set directory word directory_base "exported_ascii_MS_HABMAT_NO_SS/" ;define directory
  set direct_list sort pathdir:list directory ;list of files in directory in ascending order
  set landscape item num_lands direct_list ;get one of the landscape from the list and define as landscape
  set landscape word directory landscape ;set the new_landscape with the directory to import
end

to import_PATCHES_ONLY ;import landscape with habitat patches only
...
;-----------------------LANDSCAPE ID_TREES
  ...
  set direct_list_ID_TREES sort pathdir:list directory_ID_TREES ;list of files in directory in ascending order
  ...
  set landscape_ID_TREES word directory_ID_TREES landscape_ID_TREES
end

to import_DIST_PATCHES ;import DIST from habitat patches
;-----------------------LANDSCAPE DIST OF PATCHES
  ...
  set direct_list_DIST_PATCHES sort pathdir:list directory_DIST_PATCHES ;list of files in directory in ascending order
  ...
  set landscape_DIST_PATCHES word directory_DIST_PATCHES landscape_DIST_PATCHES
end

to import_DIST_SS ;import DIST from stepping stones
;-----------------------LANDSCAPE DIST OF SS
  set directory_DIST_SS word directory_base "exported_ascii_MS_HABMAT_SS_5HA_DIST/" ;define directory
  set direct_list_DIST_SS sort pathdir:list  directory_DIST_SS ;list of files in directory in ascending order
  ...
  set landscape_DIST_SS word directory_DIST_SS landscape_DIST_SS
end
...
;-----------------------LANDSCAPE DIST OF TREES
  ...
  set direct_list_DIST_TREES sort pathdir:list directory_DIST_TREES ;list of files in directory in ascending order
  ...
  set landscape_DIST_TREES word directory_DIST_TREES landscape_DIST_TREES
end
...
;------------------------------------------------------Set landscape
  set landscape-dataset gis:load-dataset landscape
  ...
  ask patches with [cover = 1] [ set pcolor green ] ; Green = Habitat PatchesFragments
...
;------------------------------------------------------Set landscape ID_PATCHES
  set landscape-ID_PATCHES-dataset gis:load-dataset landscape_ID_PATCHES
...
;------------------------------------------------------Set landscape ID_SS
  set landscape-ID_SS-dataset gis:load-dataset landscape_ID_SS
...
;------------------------------------------------------Set landscape ID_TREES
  set landscape-ID_TREES-dataset gis:load-dataset landscape_ID_TREES
...
;------------------------------------------------------Set landscape PATCHES_DIST
  set landscape-DIST_PATCHES-dataset gis:load-dataset landscape_DIST_PATCHES
...
;------------------------------------------------------Set landscape SS_DIST
  set landscape-DIST_SS-dataset gis:load-dataset landscape_DIST_SS
...
;------------------------------------------------------Set landscape TREES_DIST
  set landscape-DIST_TREES-dataset gis:load-dataset landscape_DIST_TREES
...
to setup ;;---------------------------------------CREATE LANDSCAPE AND TURTLES------------------------------------------------
  ...
  [set file word file "_TREES_ONLY"]
  if TREES = true and SS = true
  [set file word file "_TREES_SS"]
  if TREES = false and SS = true
  [set file word file "_SS_ONLY"]
  if SS = false and TREES = false
  [set file word file "_PATCHES_ONLY"]
  file-close
  while [file-exists? word file "_data.txt"]
  [set file word file 0]
 ;---------------------------------------------------CREATE ENVIRONMENT----------------------------------------------------
  ...
  ;--------------------------------------------DEFINE SCENARIOS (HAB(PATCHES_ONLY),HAB_SP(SS_ONLY),HAB_ST(TREES_ONLY),HAB_SP_ST(TREES_SS))

  base; SET DIRECTORY BASE

  if TREES = true and SS = true ;(patches, ss and trees)
  [
    ;import landscape with all elements
    import_TREES_SS
    ...
    ;define all
    define_landscape
    ;define ID of all elements
    define_ID_PATCHES
    define_ID_SS
    define_ID_TREES
    ;define DIST from all elements
    define_DIST_PATCHES
    define_DIST_SS
    define_DIST_TREES
  ]
  ...
  [
    ;import landscape with patches and ss
    import_SS
    ...
    ;define all
    define_landscape
    ;define ID of patches and ss
    define_ID_PATCHES
    define_ID_SS
    ;define DIST of patches and ss
    define_DIST_PATCHES
    define_DIST_SS
  ]
  ...
  [
    ...
    ;import ID of patches and trees
    import_ID_PATCHES
    import_ID_TREES
    ;import DIST from patches and trees
    import_DIST_PATCHES
    import_DIST_TREES

    ;define all
    define_landscape
    ;define ID of patches and trees
    define_ID_PATCHES
    define_ID_TREES
    ;define DIST of patches and trees
    define_DIST_PATCHES
    define_DIST_TREES
  ]
  if SS = false and TREES = false ;(only patches)
  [
    ;import landscape with patches only
    import_PATCHES_ONLY
    ;import ID of patches
    import_ID_PATCHES
    ;import DIST from patches
    import_DIST_PATCHES
  ...
  ;create a patch-set of patches with cover == 0 and DIST greater than perceptual range, that patch-set will be where the individuals will start the movement.

  if not any? patches with [cover = 0 and PATCHES_DIST > (perceptual_range) and PATCHES_DIST < (perceptual_range) + 10] [set initial_border_patches max-n-of 10000 patches [PATCHES_DIST]]
  ;when the landscape has too much forest cover, than there is no way to create a patch-set of patches with cover == 0 and DIST greater than the perceptual range, than we just create a patch-set of 10000 patches with greater DIST, e randonly distribute individuals there.
...
;------------------------------------------------------CREATE AGENTS---------------------------------------------------------
  create-turtles individuals [setxy random-xcor random-ycor]
   ask turtles [
    ...
    set source [PATCHES_ID] of initial_frag ;define fragment initial for where each turtle came from
...
to go ;;Start simulation
  ...
  if ticks = 10000 ;end of simulation time.
  [
    ask turtles [
      set time1 time1 + 1
      if [cover] of patch-here = 0
...
to move ;moving across matrix oriented or non-oriented
  ...
  [
    ...
    ;-----Calculate coordinates using l and turning angle----
    ...
    ;-----Mortality chance proportional of step lenght (l)
    mortality
    ...
    ;-----Define new coordinates (move turtle)
    setxy new-xcor new-ycor
  ]
  ;--------------------------ORIENTED MOVEMENT ON MATRIX TO PATCH OR STEPPING STONE
  [ ;near_cover - patchset of all patches that are in the radius of PR and might be tree,ss or patch different for the one before.
    ...
    [
      ...
      ;-----Calculate coordiantes ----goal_pixel with [cover = 1 or cover = 2]
      set new-xcor (xcor + (l / resolution * sin(ang))) ;definir a nova coordenada x
      set new-ycor (ycor + (l / resolution * cos(ang))) ;definir a nova coordenada y
    ...
    ;--------------------------ORIENTED MOVEMENT ON MATRIX TO TREES
...
to in_patch
  ...
  ;define next step
  ifelse [PATCHES_ID] of patch-here = [PATCHES_ID] of initial_frag and distance_total < 1000 ;If the individual arrive at the same source fragment, he might stop if he had walked already more than 1000, or he should go to matrix again
  [
...
@#$#@#$#@
<experiments>
  <experiment name="didelphis_TREES_SS_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="philander_TREES_SS_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="marmosa_TREES_SS_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="didelphis_SS_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="philander_SS_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="marmosa_SS_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="didelphis_PATCHES_ONLY_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="philander_PATCHES_ONLY_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="marmosa_PATCHES_ONLY_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="didelphis_TREES_ONLY_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="philander_TREES_ONLY_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="marmosa_TREES_ONLY_0.01" repetitions="2" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
  ...
  <experiment name="CP_DA" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles</metric>
    <enumeratedValueSet variable="specie">
    ...
    <steppedValueSet variable="perceptual_range" first="300" step="50" last="400"/>
    <enumeratedValueSet variable="mortality_rate">
      <value value="0.01"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="individuals">
